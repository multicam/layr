...
Dev server management and browser error monitoring for Layr.
...
**Starting/Restarting:**
1. Kill any existing processes on the ports defined in `layr.yaml` (default: 3000, 5173):
   ```bash
   lsof -ti:3000 -ti:5173 2>/dev/null | xargs kill -9 2>/dev/null
   ```
...
3. Start dev server in background:
   ...
   - `bun run --filter './packages/backend' dev` (Hono backend, auto-restarts on file changes)
   - `bun run --filter './packages/editor' dev` (Vite editor, HMR)
   Ports are configured in `layr.yaml` at the project root.
...
- Fix the code
- The editor should auto-refresh via Vite HMR
- Run `/dev check` again to verify the fix

## Key Details

- Port configuration lives in `layr.yaml` at the project root
- Backend (Hono): `http://localhost:3000` (SSR, routing, API proxy)
- Editor (Vite): `http://localhost:5173` (React + Zustand visual editor)
- Demo project: `http://localhost:3000/demo/`
- The backend `bun --watch` auto-restarts on ANY server file change
- The editor Vite HMR auto-refreshes on ANY frontend file change
- Only manual restart needed: env changes, dependency changes, or crashes
