/**
 * Unknown Variable Setter Rule
 * Checks for SetVariable actions targeting non-existent variables
 */

import type { Rule } from '../../types';
import type { ActionModel, EventModel, Component } from '@layr/types';

export const unknownVariableSetterRule: Rule<{ variableName: string }, ActionModel> = {
    ...
    const checkActions = (actions: ActionModel[] | undefined, component: Component, basePath: (string | number)[]): void => {
      ...
      for (let i = 0; i < actions.length; i++) {
        ...
        if (action.type === 'SetVariable') {
          ...
          if (!component.variables?.[action.name]) {
            report({ variableName: action.name }, [...basePath, i]);
          }
        }
